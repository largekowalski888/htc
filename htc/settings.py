...
class Settings:
    ...
    - `HTC_ADD_NETWORK_ALTERNATIVES`: If set to the string `true`, will include results and intermediate directories on the network drive (default `false`). This is usually only required for testing. Note: the intermediates of the datasets are not available per default because usually you want fast access to those files (e.g., during training), so you need local access to those datasets. Example: `HTC_ADD_NETWORK_ALTERNATIVES="true"`
