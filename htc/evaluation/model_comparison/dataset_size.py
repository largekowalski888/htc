...
def _run_results(run_dir: Path, metric_name: str, mapping: LabelMapping) -> pd.DataFrame:
    rows = []
...
def dataset_size_table(runs: list[Path], metric_name: str = "dice_metric") -> pd.DataFrame:
    ...
    results = pd.concat(p_map(partial(_run_results, metric_name=metric_name, mapping=mapping), runs)).sort_values(
        by=["n_pigs", "seed"]
    )
