...
class HideAndSeek(ContextTransformation):
    def __init__(
        self,
        proportion: float | tuple[float, float],
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            ...
            **kwargs: Additional keyword arguments passed to the parent classes.
        """
        super().__init__(**kwargs)
...
class SuperpixelMixin:
    def __init__(self, proportion: float | tuple[float, float], p: float = 1, **kwargs):
        """
...
class RandomMixUp(HTCTransformation):
    def __init__(self, config: Config, p: float, **kwargs):
        """
        Randomly mix images and corresponding labels inside a batch. Works best if the batch size is large enough.

        After this augmentation, the new labels will be one-hot encoded and a new key `labels_original` is added to the batch with the original integer labels.
        ...
        """
        super().__init__(**kwargs)
        self.n_classes = get_n_classes(config)
        self.p = p
        assert 0 <= self.p <= 1, f"Invalid p value: {self.p}"

    def __repr__(self) -> str:
        return f"RandomMixUp(p={self.p})"

    def __call__(self, batch: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        # The one-hot encoding must always take place when this augmentation is activated
        if "labels" in batch:
            # We need to replace the invalid labels with a "valid" one for the one-hot encoding (but the values won't be used)
            batch["labels_original"] = batch["labels"].clone()
            batch["labels"] = batch["labels"].masked_fill(~batch["valid_pixels"], 0)
            batch["labels"] = F.one_hot(batch["labels"], num_classes=self.n_classes).to(batch["features"].dtype)

        if "image_labels" in batch:
            batch["image_labels"] = F.one_hot(batch["image_labels"], num_classes=self.n_classes).to(
                batch["features"].dtype
            )

        selection = torch.rand(batch["features"].size(0), device=batch["features"].device) <= self.p
        if selection.any():
            # Weights must be broadcastable
            weights = torch.rand(selection.sum(), device=selection.device)  # [B]
            weights_features = weights.reshape(weights.shape + (1,) * len(batch["features"].shape[1:]))  # [B, 1, 1, 1]

            # Randomly select images in the batch which act as source for the interpolation
            selection_source = torch.randperm(batch["features"].size(0))[: selection.sum()]

            batch["features"][selection] = torch.lerp(
                batch["features"][selection], batch["features"][selection_source], weights_features
            )

            if "labels" in batch:
                # Interpolate between the one-hot encoded labels
                batch["labels"][selection] = torch.lerp(
                    batch["labels"][selection], batch["labels"][selection_source], weights_features
                )

                # Only pixels which are valid from both selections should be used
                batch["valid_pixels"][selection] = (
                    batch["valid_pixels"][selection] & batch["valid_pixels"][selection_source]
                )

            if "image_labels" in batch:
                weights_image_labels = weights.reshape(
                    weights.shape + (1,) * len(batch["image_labels"].shape[1:])
                )  # [B, 1]
                batch["image_labels"][selection] = torch.lerp(
                    batch["image_labels"][selection], batch["image_labels"][selection_source], weights_image_labels
                )

            if "meta" in batch:
                weights_meta = weights.reshape(weights.shape + (1,) * len(batch["meta"].shape[1:]))  # [B, 1]
                batch["meta"][selection] = torch.lerp(
                    batch["meta"][selection], batch["meta"][selection_source], weights_meta
                )
